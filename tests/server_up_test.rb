...
class TestServerUp < Test::Unit::TestCase
  ...
  def test_get_tests_successfully
    ...
    expected_response_body = JSON.parse File.read('./tests/tests_helper/test_db_data.json')
  ...
  def test_post_data_successfully
    ...
    request.body = File.read('./tests/tests_helper/test_data.csv')
  ...
  def test_post_with_invalid_data
    address = Net::HTTP.new('localhost', 3000)
    request = Net::HTTP::Post.new('/insert', 'Content-Type': 'text/csv')
    request.body = File.read('./tests/tests_helper/test_invalid_data.csv')
  ...
  def test_post_insert_multiple_times
    ...
    first_request.body = File.read('./tests/tests_helper/test_multiple_insert_data1.csv')
    second_request = Net::HTTP::Post.new('/insert', 'Content-Type': 'text/csv')
    second_request.body = File.read('./tests/tests_helper/test_multiple_insert_data2.csv')
